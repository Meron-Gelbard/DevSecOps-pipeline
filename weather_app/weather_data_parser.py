...
COUNTRY_CODES = {country: code for code, country in requests.get("https://flagcdn.com/en/codes.json", timeout=5).json().items()}
WEATHER_API_KEY = os.getenv("WEATHER_API_KEY")
LOCATION_API_KEY = os.getenv("LOCATION_API_KEY")
API_ENDPOINT = f"https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/" \
               f"timeline/ /next7days/?iconSet=icons2&unitGroup=metric&key={WEATHER_API_KEY}"
...
def parse_weather_data(country_city):
    ...
    data = requests.get(API_ENDPOINT.replace(" ", country_city), timeout=5)
...
def get_current_location():
    ...
    response = requests.get(f"https://ipgeolocation.abstractapi.com/v1/?api_key={LOCATION_API_KEY}", timeout=5).json()
    try:
